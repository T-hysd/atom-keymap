...
exports.keystrokeForKeyboardEvent = (event, customKeystrokeResolvers) ->
  ...
  if true
     # (key.length is 1 and not isLatinKeymap(KeyboardLayout.getCurrentKeymap())) or 
     # (metaKey and KeyboardLayout.getCurrentKeyboardLayout() is 'com.apple.keylayout.DVORAK-QWERTYCMD') or 
     # (metaKey and not (KeyboardLayout.getCurrentKeyboardLayout() is 'com.apple.keylayout.DVORAK-QWERTYCMD'))
